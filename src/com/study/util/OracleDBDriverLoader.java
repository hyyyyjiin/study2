...
public class OracleDBDriverLoader extends HttpServlet {
	//서블릿 딱한번 실행되는 메소드 -> init()
	@Override
   ...
   private void initConnectionPool() {
        try {
          ...
          PoolableConnectionFactory poolableConnFactory = new PoolableConnectionFactory(connFactory, null);
            //커넥션이 유효한지 확인할때 사용하는 쿼리를 설정한다.  DB를 껐다 켤때 쿼리를 날려서 잘 있는지 확인해야한다.
          poolableConnFactory.setValidationQuery("select 1 from dual");
            
            //커넥션 풀의 설정 정보를 생성한다.
            GenericObjectPoolConfig poolConfig = new GenericObjectPoolConfig();
            //유휴 커넥션 검사 주기
            poolConfig.setTimeBetweenEvictionRunsMillis(1000L * 60L * 5L); // 5분 , 이커넥션이 살아있는지 죽어있는지 확인
